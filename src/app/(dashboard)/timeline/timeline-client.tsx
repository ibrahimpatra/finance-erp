...
export function TimelineClient() {
  ...
  // Pass defaultCode so transferred events without currencyCode resolve correctly
  const allEvents = useTimeline(defaultCode);

  // Apply the global currency filter — same pattern as every other list page
  const events = allEvents.filter((e) => matches(e.currencyCode));
    ...
    <div className="space-y-5 animate-fade-in max-w-3xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Complete activity history · {events.length} event{events.length !== 1 ? "s" : ""}
          </p>
        </div>
      ...
      {events.length === 0 ? (
        <EmptyState
          ...
          description="Your income, expense, and transfer history will appear here."
        />
      ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {event.subtitle && (
                          <div className="text-xs text-muted-foreground mt-0.5 truncate">{event.subtitle}</div>
                        )}
                        <div className="text-xs text-muted-foreground/70">{formatDateTime(event.date)}</div>
                      </div>
                      <div className={`text-sm font-bold shrink-0 amount-display ${
                        isCredit
                          ? "text-emerald-600"
                          : event.type === "expense"
                          ? "text-red-500"
                        ...
                        {/* FIX: was formatCurrency(event.amount) — always used base currency.
                            Now uses formatFor(amount, currencyCode) for the event's actual currency. */}
                        {isCredit ? "+" : "−"}{formatFor(event.amount, event.currencyCode)}
                      </div>
                    </Link>
                  );
                })}
