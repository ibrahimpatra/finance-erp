...
import {
  ...
  deleteExpense, createRefund, reassignExpense,
} from "@/services/expense.service";
import { runFIFOAttribution, resolveFIFOPrimaryIncome } from "@/services/attribution.service";
...
export const useExpenseStore = create<ExpenseStore>((set, get) => ({
  ...
  addExpense: async (userId, data) => {
    ...
    // ── Auto FIFO attribution (display-only, never affects balance) ──
    // Runs in background after save. Any error is swallowed so it never
    // surfaces to the user — attribution is supplementary display data only.
    if (data.accountId && data.incomeSourceId) {
      // Lazily import income store to avoid circular dependency at module load time
      void (async () => {
        try {
          const { useIncomeStore } = await import("@/stores/income.store");
          const incomes = useIncomeStore.getState().incomes;
          const accountIncomes = incomes.filter((i) => i.accountId === data.accountId);
          if (accountIncomes.length > 0) {
            await runFIFOAttribution(
              userId,
              data.accountId!,
              id,
              data.amount,
